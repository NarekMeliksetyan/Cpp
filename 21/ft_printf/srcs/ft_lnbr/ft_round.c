...
/*   Updated: 2019/12/08 18:00:56 by jkrypto          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_lnbr	ft_prepare(register t_lnbr sing, register t_lnbr fract,
							size_t prec)
{
	size_t			i;
	size_t			j;
...
static t_lnbr	ft_return_zeros(register t_lnbr pre, register t_lnbr nbr)
{
	...
	i = 0;
	j = -1;
	res = ft_numzero(pre.len);
	while (pre.num[i] == '0')
		i++;
	if (nbr.num[0] == '1' && pre.num[i] == '9')
		i--;
	while (i + ++j < pre.len)
						...
						size_t prec)
{
	...
	add = ft_numzero(2);
	add.num[0] = '5';
	pre = ft_prepare(sing, fract, prec);
